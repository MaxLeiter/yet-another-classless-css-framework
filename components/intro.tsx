const Intro = () => <>
    <p>
        This is a demo for Yet Another Classless CSS Framework, the framework I
        use for my personal websites and as a base for my side projects. To see
        the CSS behind any feature you can click the details dropdown component in each section.
    </p>
    <p>
        You can find on the source on <a href="https://github.com/MaxLeiter/yet-another-classless-css-framework/" rel="noopener noreferrer" target="_blank">GitHub</a>.
    </p>

    <a href="#features"><h2 id="features">Features</h2></a>
    <ul>
        <li>Embraces semantic html</li>
        <li>Responsive</li>
        <li>Uses CSS variables</li>
        <li>Advanced features via CSS attributes</li>
        <ul>
            <li>For example, hover over a heading element below.</li>
        </ul>
        <li>Supports light and dark mode</li>
        <li>Modular and lightweight</li>
    </ul>
</>

export default Intro